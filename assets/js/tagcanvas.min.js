...
(function() {
	...
	R.Rotation = function(aI, i) {
		var j = ah(aI),
			aH = w(aI),
			aJ = 1 - aH;
		return new R([aH + at(i.x, 2) * aJ, i.x * i.y * aJ - i.z * j, i.x * i.z * aJ + i.y * j, i.y * i.x * aJ + i.z * j,
			aH + at(i.y, 2) * aJ, i.y * i.z * aJ - i.x * j, i.z * i.x * aJ - i.y * j, i.z * i.y * aJ + i.x * j, aH + at(i.z,
				2) * aJ
		])
	};
	aF.mul = function(aH) {
	...
	function Y(aI, aH) {
		...
		return (j && j.getComputedStyle && j.getComputedStyle(aI, null).getPropertyValue(aH)) || (aI.currentStyle && aI.currentStyle[
			i])
	}
	...
	function u(j, aI, aH) {
		...
		if (aI) {
			...
			if (aJ = Y(j, "font-size")) {
				i = (aJ.indexOf("px") > -1 && aJ.replace("px", "") * 1) || (aJ.indexOf("pt") > -1 && aJ.replace("pt", "") * 1.25) ||
					aJ * 3.3
			}
		}
		return i
	...
	c.Measure = function(aM, aP) {
		...
		if (aP.txtOpt) {
			...
			aI = aK.Create(this.colour, this.bgColour, this.bgOutline, aQ * this.bgOutlineThickness, aP.shadow, aQ * aP.shadowBlur,
				j, aQ * this.padding, aQ * this.bgRadius);
			if (aP.outlineMethod == "colour") {
				this.oimage = aK.Create(aP.outlineColour, this.bgColour, aP.outlineColour, aQ * this.bgOutlineThickness, aP.shadow,
					aQ * aP.shadowBlur, j, aQ * this.padding, aQ * this.bgRadius)
			} else {
				if (aP.outlineMethod == "size") {
					...
					if (this.image) {
						aK.SetImage(this.image, this.iw + aP.outlineIncrease, this.ih + aP.outlineIncrease, aP.imagePosition, aP.imagePadding,
							aP.imageAlign, aP.imageVAlign, aP.imageScale)
					}
					this.oimage = aK.Create(this.colour, this.bgColour, this.bgOutline, aQ * this.bgOutlineThickness, aP.shadow, aQ *
						aP.shadowBlur, j, aQ * this.padding, aQ * this.bgRadius);
	...
	function y(aN, j, aI) {
		...
		if (!y.started) {
			aL = window.requestAnimationFrame = window.requestAnimationFrame || window.mozRequestAnimationFrame || window.webkitRequestAnimationFrame ||
	...
	Q.CreateTag = function(aL) {
		...
		if (aN || aJ) {
			aP = new e(this, aK, aL, j, 2, this.textHeight + 2, this.textColour || Y(aL, "color"), aM, this.bgRadius, aI, this
				.bgOutlineThickness, aH, this.padding, aN && aN.original);
			if (aJ) {
				aP.SetImage(aJ);
				aw(aJ, aO[0], aP, this)
			} else {
